...
class MixtralDecoderLayer(nn.Module):
    def __init__(self, config: MixtralConfig, layer_idx: int):
        ...
        if hasattr(config, 'betas'):
            assert isinstance(config.betas, dict) and len(config.betas) == config.num_hidden_layers
            self.block_sparse_moe = DynamicSkippingMixtralSparseMoeBlock(config, config.betas[str(layer_idx)])
            warnings.warn(
